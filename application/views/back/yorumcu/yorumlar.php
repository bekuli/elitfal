    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Yorum Görüntüle</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(".urltable-content").load("<?=base_url()?>yorumcu/yorumlar_list");
        ...
        $(".urltable-content").on("click", "a[data-action]", function(e){
            ...
            if ($(this).attr("data-action") == "view"){
                $.ajax({
                    url: '<?=base_url()?>yorumcu/yorumlar/' + $(this).parent().attr("data-id") + "/view",
                    contentType: false,
                    processData: false,
                    beforeSend: function(){
        ...
        $("#delete-yes").click(function(){
            $.ajax({
                url: '<?=base_url()?>yorumcu/yorumlar/' + $("input[name='delete-id']").attr("value") + "/delete",
                contentType: false,
                processData: false,
                success: function( data){
        ...
        $(".urltable-content").on("click", ".urltable-pagination a",function(e){
            ...
            if ($(this).attr("page") != "active")
            {
                ...
                if (typeof q !== typeof undefined && q !== false)
                    $(".urltable-content").load("<?=base_url()?>yorumcu/yorumlar_list/" + $(this).attr("page") + "/" + $(this).attr("search"));
                else
                    $(".urltable-content").load("<?=base_url()?>yorumcu/yorumlar_list/" + $(this).attr("page"));
            }
        });

        $("#search-url").submit(function(e){
            e.preventDefault();
            $(".urltable-content").load("<?=base_url()?>yorumcu/yorumlar_list/1/"+$("#search-url input").val());
        });
    });
